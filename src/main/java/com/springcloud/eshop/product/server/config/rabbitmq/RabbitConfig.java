...
public class RabbitConfig {
    ...
    public RabbitAdmin rabbitAdmin(ConnectionFactory connectionFactory) {
        ...
        //数据同步的队列 data sync
        DirectExchange dataSyncExchange = new DirectExchange(RabbitConstants.MQ_EXCHANGE_DATA_SYNC_QUEUE);
        //声明队列
        Queue dataSyncQueue = queue(RabbitConstants.QUEUE_NAME_DATA_SYNC);
        rabbitAdmin.declareQueue(dataSyncQueue);
        rabbitAdmin.declareExchange(dataSyncExchange);
        rabbitAdmin.declareBinding(BindingBuilder.bind(dataSyncQueue).to(dataSyncExchange).with(RabbitConstants.MQ_ROUTING_KEY_DATA_SYNC));
